...
{
	private readonly string DragDropDataDirectory = $"Data{Path.DirectorySeparatorChar}DragDrop";
	private const string NyanCatAsciiTxt = @"NyanCatAscii.txt";
	private const string DragAcceptRtf = @"DragAccept.rtf";
	private readonly string NyanCatAscii = string.Empty;
	private readonly Bitmap _dragAcceptBmp = new ($"Data{Path.DirectorySeparatorChar}DragDrop{Path.DirectorySeparatorChar}DragAccept.bmp");
	private readonly Bitmap _nyanCatAscii301Bmp = new ($"Data{Path.DirectorySeparatorChar}DragDrop{Path.DirectorySeparatorChar}NyanCatAscii_301.bmp");
	private readonly Bitmap _nyanCatBmp = new ($"Data{Path.DirectorySeparatorChar}DragDrop{Path.DirectorySeparatorChar}NyanCat1.bmp");
	private readonly Bitmap _toolStripAsciiCatBmp = new ($"Data{Path.DirectorySeparatorChar}DragDrop{Path.DirectorySeparatorChar}ToolStripAsciiCat.bmp");
	private readonly Bitmap _toolStripDragAcceptBmp = new ($"Data{Path.DirectorySeparatorChar}DragDrop{Path.DirectorySeparatorChar}ToolStripDragAccept.bmp");
	private readonly Bitmap _toolStripNyanCatBmp = new ($"Data{Path.DirectorySeparatorChar}DragDrop{Path.DirectorySeparatorChar}ToolStripNyanCat.bmp");
	...
	private void PictureBox_MouseDown(object? sender, MouseEventArgs e)
	{
		if (sender is PictureBox pictureBox)
		{
			...
			pictureBox.DoDragDrop(data, DragDropEffects.All);
		}
	}
	...
	private void TextBox_DragDrop(object? sender, DragEventArgs e)
	{
		if (e.Data is not null
				...
				&& fileNames.Length > 0 && fileNames[0].Contains(nameof(NyanCatAscii)))
		{
			textBox.Text += textBox.Text.Length > 0
							? Environment.NewLine + Environment.NewLine + NyanCatAscii
							: NyanCatAscii;
		}
	...
	private void TextBox_DragEnter(object? sender, DragEventArgs e)
	{
		...
		if (e.Data.GetDataPresent(DataFormats.FileDrop)
				...
				&& files.Length > 0)
		{
			//if (e.Data.GetDataPresent(nameof(NyanCatAscii)))
			if (files[0].Contains(nameof(NyanCatAscii)))
			{
				// Set the target drop image to a plus sign (+).
				//e.DropImageType = DropImageType.Copy;
				// Set the target drop text.
			...
			else// if (files[0].Contains(nameof(_nyanCatBmp)))
				//if (e.Data.GetDataPresent(nameof(_nyanCatBmp)))
			{
				//e.DropImageType = DropImageType.None;
				//e.Message = "Nyan %1";
				//e.MessageReplacementToken = "Cat";
	...
	private void RichTextBox_DragEnter(object? sender, DragEventArgs e)
	{
		...
		if (e.Data.GetDataPresent(DataFormats.FileDrop)
				...
				&& files.Length > 0)
		{
			if (files[0].Contains("DragAccept.rtf"))
			{
			...
			else// if (e.Data.GetDataPresent(nameof(NyanCatAscii)))
			{
				//e.DropImageType = DropImageType.None;
				//e.Message = "Ascii %1";
				//e.MessageReplacementToken = "Cat";
	...
	private string ReadAsciiText()
	{
		...
		{
			MessageBox.Show($"Error reading ascii text: {ex.Message}", "Drag & Drop Test", MessageBoxButtons.OK, MessageBoxIcon.Error);
			return string.Empty;
		}
	}
	...
	private void DragAcceptItem_MouseDown(object? sender, MouseEventArgs e)
	{
		if (sender is ToolStripItem toolStripItem)
		{
			...
			toolStripItem.DoDragDrop(data, DragDropEffects.All);
		}
	}
